...
- Copy all the files from coinor-coinutils to CoinUtils-2.11.3 (for example)
- In CoinUtils-2.11.3 create a build directory (mkdir build)
- In CoinUtils-2.11.3/build, launch 'cmake -DCMAKE_INSTALL_PREFIX=/opt/coinutils-2.11.3 -DCMAKE_BUILD_TYPE=RELEASE ..'
- In CoinUtils-2.11.3/build, launch 'cmake-gui .' to fine tune the configuration
- Make the libraries: make
- Install the libraries: sudo make install

An example:
```
$ wget https://www.coin-or.org/download/source/CoinUtils/CoinUtils-2.11.3.tgz
$ tar xvfz CoinUtils-2.11.3.tgz
$ git clone https://github.com/ycollet/coinor-coinutils.git
$ cp -r coinor-coinutils/* CoinUtils-2.11.3/
$ cd CoinUtils-2.11.3
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=/opt/coinutils-2.11.3 -DCMAKE_BUILD_TYPE=RELEASE ..
...
- Copy all the files from coinor-coinutils to CoinUtils-2.11.3 (for example)
- In CoinUtils-2.11.3 create a build directory (mkdir build)
- In CoinUtils-2.11.3/build, launch 'cmake -DCMAKE_INSTALL_PREFIX=/opt/coinutils-2.11.3 -DCMAKE_BUILD_TYPE=RELEASE -G "NMake Makefiles" ..'
- In CoinUtils-2.11.3/build, launch 'cmake-gui .' to fine tune the configuration
...
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=~/coinutils-bin -DCMAKE_BUILD_TYPE=DEBUG -DCMAKE_CXX_COMPILER=/usr/bin/clang -DCMAKE_CXX_FLAGS="-fsanitize=memory" ..
# or
$ cmake -DCMAKE_INSTALL_PREFIX=~/coinutils-bin -DCMAKE_BUILD_TYPE=DEBUG -DCMAKE_CXX_COMPILER=/usr/bin/clang++ -DCMAKE_C_COMPILER=/usr/bin/clang \ 
...
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=~/coinutils-bin -DCMAKE_BUILD_TYPE=DEBUG -DCMAKE_CXX_COMPILER=/opt/intel/bin/icpc -DCMAKE_C_COMPILER=/opt/intel/bin/icc ..
$ make
```
...
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=d:/coinutils-bin -DCMAKE_BUILD_TYPE=DEBUG -G "NMake Makefiles" ..
$ nmake
```
...
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=/home/me/coinutils-bin -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_TOOLCHAIN_FILE=cmake/mingw64-toolchain.cmake ..
$ make
```
...
$ cp -r ../coinor-coinutils/* .
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=/opt/coinutils-devel -DCMAKE_BUILD_TYPE=RELEASE ..
$ make
...
$ cp -r ../coinor-coinutils/* CoinUtils/
$ cd CoinUtils
$ mkdir build
$ cd build
$ cmake -DCMAKE_INSTALL_PREFIX=/opt/coinutils-devel -DCMAKE_BUILD_TYPE=RELEASE ..
